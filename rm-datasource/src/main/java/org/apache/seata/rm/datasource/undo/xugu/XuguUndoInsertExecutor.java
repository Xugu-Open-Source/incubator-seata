 ...
 */
package org.apache.seata.rm.datasource.undo.xugu;
 ...
 *
 */
public class XuguUndoInsertExecutor extends AbstractUndoExecutor {
    ...
    private String generateDeleteSql(List<Row> rows, TableRecords afterImage) {
        List<String> pkNameList = getOrderedPkList(afterImage, rows.get(0), JdbcConstants.XUGU).stream()
                .map(e -> e.getName())
                .collect(Collectors.toList());
        String whereSql = SqlGenerateUtils.buildWhereConditionByPKs(pkNameList, JdbcConstants.XUGU);
     ...
     */
    public XuguUndoInsertExecutor(SQLUndoLog sqlUndoLog) {
        super(sqlUndoLog);
    }
